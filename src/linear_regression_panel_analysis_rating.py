...
class LinearRegressionPanelAnalysis:
    ...
    def run_analysis(self):
        ...
        # Filter data for panel based on the 'rating' column
        panel_filtered = panel[panel['rating'].between(8, 16)]

        # Run simple regression for filtered panel
        self.run_simple_regression(
            panel_filtered,
            os.path.join(self.output_path, "rating_simple_regression_results.txt")
        )

        # Run Huber regression for panel
        self.run_huber_regression(
            panel_filtered,
            os.path.join(self.output_path, "rating_huber_regression_results.txt"), epsilon=1.35
        )
